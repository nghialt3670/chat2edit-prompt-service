...
from database.models import Conversationersation
...
class ConvService:
    ...
    def find_one_by_user_id(self, user_id: ObjectId) -> Optional[Conversationersation]:
        return self._collection.find_one({"user_id": ObjectId(user_id)})

    def find_by_user_id(
        self, user_id: ObjectId, limit: int = -1, offset: int = 0
    ) -> List[Conversationersation]:
        cursor = self._collection.find({"user_id": ObjectId(user_id)}).skip(offset)

        if limit > 0:
            cursor = cursor.limit(limit)

        convs = [Conversationersation(**doc) for doc in cursor]
        return convs

    def cache(self, conv: Conversationersation) -> None:
        if self._redis is None:
            raise RuntimeError("No Redis provided")

        self._redis.set(str(conv.id), conv.json())

    def save(self, conv: Conversationersation, cache: bool = True) -> None:
