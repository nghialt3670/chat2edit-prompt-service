...
class OpenAILLM(LLM):
    ...
    def _create_input_messages(self, messages: Iterable[str]) -> List[str]:
        ...
        for req_message, llm_message in zip(messages[::2], messages[1::2]):
            input_messages.append({"role": "user", "content": req_message})
            input_messages.append({"role": "assistant", "content": llm_message})
